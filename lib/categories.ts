...
export async function getCategory(id: number) {
  const category = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/categories?id=${id}`, {
    method: "GET",
...
export async function createCategory(category: Omit<Category, "category_id">) {
  const newCategory = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/categories`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function deleteCategory(id: number) {
  const deletedCategory = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/categories?id=${id}`, {
    method: "DELETE",
    headers: { "Content-Type": "application/json" },
  });
...
export async function updateCategory(category: Category) {
  const updatedCategory = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/categories?id=${category.category_id}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(category),
  });
