...
export default function CarFormGallery({ handleAddImage, car, setCar }: CarFormGalleryProps) {
  ...
  async function handleDelete(picture: Car_picture) {
    ...
    setCar({
      ...car,
      car_pictures: car.car_pictures.filter((p) => p.car_picture_id !== picture.car_picture_id),
    });

    //delete picture on supabase & on db
    const response = await deleteCarPicture(picture.car_picture_id, picture.car_picture_fileKey);
  ...
  return (
    <FormBox title="Galerie de photos" defaultOpen={true}>
      {/* select photo */}
      <FormFile label="Sélectionner une photo" name="carPicture" onlinePath={`cars`} handleAddImage={handleAddImage} />
      {/* gallery */}
      <div className="flex flex-wrap">
        {car.car_pictures.map((picture) => (
          <div className="relative h-[150px] w-[150px] group m-1" key={picture.car_picture_id}>
            <Image
              priority
            ...
            <div className="absolute left-1/2 -translate-x-1/2 transform bottom-2 hidden group-hover:flex">
              <FormDelete handleDelete={() => handleDelete(picture)} textConfirm="Suppression ?" />
            </div>
          </div>
        ))}
      </div>
